  ...
  return (
    <div className="p-4 gap-2 flex flex-col relative max-w-5xl mx-auto">
      {data.pages[0].pages > 0 ? (
        ...
        <div className="min-h-[70vh] w-full text-center flex p-4 justify-center items-center">
          Start Following People to catch up...
        </div>
      )}
...
async function fetchAllPosts({ pageParam }: any) {
  const response = await axios.get(`/api/posts/following?page=${pageParam}`);
